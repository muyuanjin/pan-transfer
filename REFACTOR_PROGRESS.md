...
_Last updated: 2025-11-08 02:15 (UTC-8)_
...
2. ~~**Tighten `PanelDomRefs`.** After the binder removal, drop the proxy catch-all/index signature in `src/content/types.ts`, expose explicit getters for the few remaining DOM hooks (edge/transfer/base-dir), and fail fast when a controller asks for an unregistered key. Update all controllers/binders accordingly. Definition of done: TypeScript errors whenever arbitrary keys are accessed, and `npm run check` still passes.~~ **Completed 2025-11-07 07:08 (UTC-8):** `panelDom` now only exposes accessor methods (`get`/`set`/`ensure`), the panel shell registers refs via `assignAll`, specs bind DOM hooks through `panelDom.set`, and `npm run check` stayed green.
3. ~~**Backfill controller specs.** Add Vitest suites for `panel-edge-controller` and `runtime/transfer/transfer-controller` covering pointer hide/show, pin button sync, transfer state transitions, retries, and toast emission. Guard DOM references with test doubles so future refactors remain safe. Definition of done: new specs live under `src/content/controllers/__tests__` (or similar), run inside `npm run test`, and fail without the new behaviours.~~ **Completed 2025-11-07 09:53 (UTC-8):** Expanded the panel edge tests to cover pinned pointerdown events and missing pin-button refs, extended the transfer controller suite with progress handling + toolbar toggle assertions, and reran `npm run check` (format → vue-tsc ×2 → ESLint → Vite build → Vitest ×15 → Playwright ×3) to keep coverage green.
4. ~~**Stand up messaging integration tests.** Build a lightweight harness that simulates Chrome runtime messages hitting `src/background/index.ts`, asserting that transfer progress + history detail/delete flows update the content store correctly. These tests unblock stricter host permissions and alarm retries. Definition of done: integration suite runs as part of `npm run test` (or a new npm script referenced in `npm run check`).~~ **Completed 2025-11-08 01:08 (UTC-8):** added a Chrome runtime/storage harness plus `src/background/index.integration.spec.ts`, covering transfer-progress broadcasts along with history detail + delete flows, and wired the suite into `npm run test`.
5. ~~**Re-verify on live CHAOSPACE pages.** After the above refactors land, load the MV3 build in Chrome, exercise panel mount, transfer, history detail, settings import/export, and re-run at least one real transfer focusing on `season-manager` sanitization. Capture findings for the next PR description.~~ **Completed 2025-11-08 02:02 (UTC-8):** Loaded the MV3 build in Chrome, exercised panel mount/history/detail, ran a live transfer (season-manager sanitization verified), and validated settings import/export; findings captured for the next PR notes.
...
- Live MV3 verification (2025-11-08 02:02 UTC-8): Installed the latest build in Chrome, confirmed the panel mounts cleanly, collapsing history when transferring keeps logs visible, and a real transfer completed with sanitized season directories; also re-tested settings import/export on CHAOSPACE pages without `[Chaospace Transfer]` errors.
- Messaging integration harness added (2025-11-08 01:08 UTC-8): created a jsdom Chrome stub that mirrors runtime/tabs/storage semantics, introduced `src/background/index.integration.spec.ts` to cover transfer-progress propagation plus history detail/delete flows against the real background entry, and ran `npm run test` to ensure the new suite lives inside the primary pipeline.
- Controller specs fortified (2025-11-07 09:53 UTC-8): expanded `panel-edge-controller.spec.ts` with pinned pointerdown + pin-button-missing fallbacks, added progress/toolbar toggle coverage to `runtime/transfer/transfer-controller.spec.ts`, and reran `npm run check` (format → vue-tsc ×2 → ESLint → Vite build → Vitest ×15 → Playwright ×3) for the new expectations.
...
| P1       | Done    | Content Runtime | Remove catch-all index signatures from `PanelDomRefs` / `DetailDomRefs` and require binders/controllers to enumerate DOM hooks.                    | PanelDom accessors now use typed getters/ensure, HistoryDetailDom refs are explicit, and specs reset typed stubs (2025-11-07 10:31 UTC-8). |
| P1       | Done    | Testing         | Extend controller specs to cover `panel-edge-controller` and `runtime/transfer/transfer-controller`.                                               | Panel-edge + transfer controller suites now guard pointer hide, progress, retries, and toasts (2025-11-07).   |
| P1       | Done    | Messaging       | Introduce integration tests simulating Chrome runtime messaging for transfer progress + history detail/delete flows.                               | `src/background/index.integration.spec.ts` now covers transfer progress + history detail/delete (2025-11-08). |
| P1       | Done    | Settings        | Add DOM-level tests for file filter & rename editors to verify parsing, validation, and serialization paths.                                       | New Vitest DOM suites cover file filter/rename editors (parsing, validation, serialization) (2025-11-07 10:31 UTC-8). |
...
- 2025-11-08 02:02 (UTC-8) — Manual Chrome verification — PASS (panel mount, history detail, live transfer, settings import/export on CHAOSPACE pages).
- 2025-11-08 01:08 (UTC-8) — `npm run test` — PASS (Vitest ×15 including new background/content integration suite).
- 2025-11-07 09:53 (UTC-8) — `npm run check` — PASS (Prettier → vue-tsc ×2 → ESLint → Vite build → Vitest ×15 → Playwright ×3 URLs).
