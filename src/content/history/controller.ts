...
export function createHistoryController(deps: HistoryControllerDeps) {
  ...
  function updateHistorySelectionSummary(filtered: HistoryGroup[] | null = null): void {
    ...
    const selectedWithinFilter = groups.filter(group => state.historySelectedKeys.has(group.key)).length;
    if (panelDom.historySelectionCount) {
      panelDom.historySelectionCount.textContent = selectedWithinFilter > 0
        ? `已选 ${selectedWithinFilter} / ${total}`
        : `共 ${total}`;
    }
  ...
  async function openHistoryDetail(groupKey: string, overrides: HistoryDetailOverridesInput = {}): Promise<void> {
    ...
    const pageUrl = overridePageUrl || (typeof fallback.pageUrl === 'string' ? fallback.pageUrl : '');
  ...
  async function handleHistoryBatchCheck(): Promise<void> {
    ...
    for (let index = 0; index < candidates.length; index += 1) {
      ...
      try {
        ...
        const response = await triggerHistoryUpdate(targetUrl, null, { silent: true, deferRender: true });
        if (!response || response.ok === false) {
          failed += 1;
          continue;
        }
  ...
  function updateHistoryRateLimit(rateMs: number): void {
    const clamped = Number.isFinite(rateMs) ? Math.max(500, Math.min(rateMs, HISTORY_BATCH_RATE_LIMIT_MS * 12)) : HISTORY_BATCH_RATE_LIMIT_MS;
