...
type SafeStorageSetFn = (entries: Record<string, unknown>, contextLabel?: string) => Promise<void> | void;
...
export interface SettingsModalHandles {
  ...
  applySettingsUpdate: (nextSettings: SettingsUpdatePayload, options?: { persist?: boolean }) => SettingsSnapshot & { themeChanged: boolean };
...
function extractSettingsFormValues(
  ...
  const useTitle = domRefs.useTitleCheckbox ? domRefs.useTitleCheckbox.checked : state.useTitleSubdir;
  const useSeason = domRefs.useSeasonCheckbox ? domRefs.useSeasonCheckbox.checked : state.useSeasonSubdir;
  ...
  const presetList = presetsText
    ...
    .filter(Boolean);
  const rateInput = domRefs.historyRateInput ? Number.parseFloat(domRefs.historyRateInput.value) : Number.NaN;
...
export function createSettingsModal(options: CreateSettingsModalOptions): SettingsModalHandles {
  ...
  function applySettingsUpdate(
    ...
    const baseDir = typeof baseDirValue === 'string'
      ? normalizeDir(baseDirValue)
      : state.baseDir;
    const useTitleValue = nextSettings['useTitleSubdir'];
    const useTitle = typeof useTitleValue === 'boolean'
      ? useTitleValue
  ...
  async function importFullBackup(payload: unknown): Promise<void> {
    ...
    const panelData = (data['panel'] && typeof data['panel'] === 'object') ? data['panel'] as Record<string, unknown> : {};
