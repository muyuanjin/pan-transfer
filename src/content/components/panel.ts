...
export async function mountPanelShell(options: MountPanelShellOptions): Promise<MountedPanelShell> {
  ...
  const applyEdgeHiddenPosition = (): void => {
    ...
    const targetLeft = panelState.edgeState.side === 'left'
      ? -(panel.offsetWidth - peek)
      : window.innerWidth - peek;
  ...
  const applyPanelSize = (width?: number, height?: number): { width: number; height: number } | null => {
  ...
  const applyPanelPosition = (left?: number, top?: number): { left: number; top: number } => {
    ...
    const safeLeft = clamp(Number.isFinite(left ?? NaN) ? (left as number) : fallbackLeft, PANEL_MARGIN, maxLeft);
    const safeTop = clamp(Number.isFinite(top ?? NaN) ? (top as number) : fallbackTop, PANEL_MARGIN, maxTop);
  ...
  const savedState = await safeStorageGet<Record<string, PanelSizeSnapshot | PanelPositionSnapshot>>(
    [POSITION_KEY, SIZE_KEY],
    'panel geometry'
  );
  ...
  panelDom.settingsBaseDir = panel.querySelector<HTMLInputElement>('[data-role="settings-base-dir"]');
  panelDom.settingsUseTitle = panel.querySelector<HTMLInputElement>('[data-role="settings-use-title"]');
  panelDom.settingsUseSeason = panel.querySelector<HTMLInputElement>('[data-role="settings-use-season"]');
  panelDom.settingsTheme = panel.querySelector<HTMLSelectElement>('[data-role="settings-theme"]');
  panelDom.settingsPresets = panel.querySelector<HTMLTextAreaElement>('[data-role="settings-presets"]');
  panelDom.settingsHistoryRate = panel.querySelector<HTMLInputElement>('[data-role="settings-history-rate"]');
  panelDom.settingsExportConfig = panel.querySelector<HTMLButtonElement>('[data-role="settings-export-config"]');
  panelDom.settingsExportData = panel.querySelector<HTMLButtonElement>('[data-role="settings-export-data"]');
  panelDom.settingsImportConfigTrigger = panel.querySelector<HTMLButtonElement>('[data-role="settings-import-config-trigger"]');
  panelDom.settingsImportDataTrigger = panel.querySelector<HTMLButtonElement>('[data-role="settings-import-data-trigger"]');
  panelDom.settingsImportConfigInput = panel.querySelector<HTMLInputElement>('[data-role="settings-import-config"]');
  panelDom.settingsImportDataInput = panel.querySelector<HTMLInputElement>('[data-role="settings-import-data"]');
  panelDom.settingsResetLayout = panel.querySelector<HTMLButtonElement>('[data-role="settings-reset-layout"]');
  ...
  panelDom.historySummaryBody = panel.querySelector<HTMLElement>('[data-role="history-summary-body"]');
  ...
  panelDom.historySelectAll = panel.querySelector<HTMLInputElement>('[data-role="history-select-all"]');
  panelDom.historySelectionCount = panel.querySelector<HTMLElement>('[data-role="history-selection-count"]');
  panelDom.historyBatchCheck = panel.querySelector<HTMLButtonElement>('[data-role="history-batch-check"]');
  panelDom.historyDeleteSelected = panel.querySelector<HTMLButtonElement>('[data-role="history-delete-selected"]');
  ...
  panelDom.historyToggleButtons = Array.from(panel.querySelectorAll<HTMLButtonElement>('[data-role="history-toggle"]'));
  ...
  const startDrag = (event: MouseEvent) => {
    ...
    if (target?.closest('button') || target?.closest('input') || target?.closest('.chaospace-theme-toggle')) {
      return;
    }
    cancelEdgeHide({ show: true });
    panelState.edgeState.isHidden = false;
