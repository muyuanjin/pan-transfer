...
export interface ResourceListRendererParams {
  ...
  computeSeasonTabState: (options?: { syncState?: boolean }) => { tabItems: unknown[]; activeId: string | null; activeTab?: any };
...
export function createResourceListRenderer(
  ...
  function renderResourceSummary(context: ResourceSummaryContext = {}): void {
    ...
    let currentVisibleCount = typeof context.visibleCount === 'number' ? context.visibleCount : total;
    let currentVisibleSelected = typeof context.visibleSelected === 'number' ? context.visibleSelected : selected;
  ...
  function renderResourceList(): void {
    ...
    const filteredItems = Array.isArray(tabState.tabItems) && tabState.tabItems.length > 0
...
function sortItems(items: ResourceItem[], sortKey: ContentState['sortKey'], sortOrder: ContentState['sortOrder']): ResourceItem[] {
  const sorted = [...items];
  if (sortKey === 'title') {
    sorted.sort((a, b) => {
...
function toResourceListItemView(item: ResourceItem, options: ResourceViewOptions): ResourceListItemView {
