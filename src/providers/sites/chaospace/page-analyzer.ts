...
class ChaospaceClassifier implements ClassificationCache {
  ...
  async getDetailedClassification(): Promise<DetailedClassificationResult> {
    ...
    let confidence = primaryAnalysis.hasJapaneseChannel
      ? 0.9
      : primaryAnalysis.hasJapaneseKeywordsStrong
        ? 0.75
        : 0.6
    ...
    if (finalClassification !== 'anime' && result.debug.isSeason) {
      ...
      if (mainDoc && mainDoc !== document) {
        ...
        if (mainAnalysis.classification === 'anime') {
          ...
          confidence = mainAnalysis.hasJapaneseChannel
            ? 0.9
            : mainAnalysis.hasJapaneseKeywordsStrong
              ? 0.75
              : 0.6
          if (mainAnalysis.hasJapaneseChannel) {
            result.reasons.push('主剧集页检测到日本电视台/平台标识')
          } else if (mainAnalysis.hasJapaneseKeywordsStrong) {
            const mainSourceLabel = mainAnalysis.hasStrongKeywordsInBody
              ? '主剧集页正文'
              : '主剧集页标题/简介'
  ...
  analyzeDocument(doc: Document): DocumentAnalysis {
    ...
    const classification: AnimeOrTvShow =
      hasJapaneseChannel || hasJapaneseKeywordsStrong ? 'anime' : 'tvshow'
    return {
      classification,
      tvChannels: channels,
      hasJapaneseChannel,
  ...
  async getMainPageContent({
    ...
    if (
      onlyWhenNecessary &&
      currentAnalysis &&
      (currentAnalysis.hasJapaneseChannel || currentAnalysis.hasJapaneseKeywordsStrong)
    ) {
      return document
    }
    ...
    try {
      ...
      chaosLogger.warn('[Pan Transfer] Failed to load main page for classification', error)
      return document
    }
  }
  ...
  private extractPageText(doc: Document): string {
    try {
      ...
      chaosLogger.warn('[Pan Transfer] Failed to extract page text', error)
      return ''
    }
  }
  ...
  private extractDescription(doc: Document): string {
    try {
      ...
      chaosLogger.warn('[Pan Transfer] Failed to extract description', error)
      return ''
    }
  }
}
...
export async function getPageClassification({
  ...
  if (!pageClassificationPromise) {
    ...
    pageClassificationPromise = classifier.getDetailedClassification().catch((error) => {
      chaosLogger.error('[Pan Transfer] Page classification failed', error)
      return {
        url: window.location.href || '',
        classification: 'unknown',
        confidence: 0,
...
export async function fetchSeasonDetail(
  info: DeferredSeasonInfo | null | undefined,
  ...
  try {
    ...
    chaosLogger.error('[Pan Transfer] Failed to load season page', info.url, error)
  }
  return {
    items: seasonItems,
    completion,
  ...
  try {
    ...
    const normalizedItems: ResourceItem[] = dedupedItems.map((item) => {
      const normalizedLabel =
        ...
        Number.isFinite(item.seasonIndex)
    ...
    chaosLogger.error('[Pan Transfer] Failed to collect links', error)
...
async function collectTvShowSeasonItems(
  options: { defer?: boolean; initialBatchSize?: number } = {},
): Promise<SeasonCollectionResult> {
  const { defer = false, initialBatchSize = TV_SHOW_INITIAL_SEASON_BATCH } = options
  ...
  const completionSummary = completionEntries.length
    ? summarizeSeasonCompletion(completionEntries)
    : null
...
function locateBaiduPanRows(root: Document | Element = document): HTMLElement[] {
  const scope =
    root && typeof (root as Element).querySelector === 'function' ? (root as Element) : document
...
export function extractItemsFromDocument(
  ...
  rows.forEach((row) => {
    ...
    const linkUrl =
      initialLinkUrl && initialLinkUrl.includes('/links/')
        ? initialLinkUrl
        : fallbackLinkUrl && fallbackLinkUrl.includes('/links/')
          ? fallbackLinkUrl
          : ''
    if (!linkUrl) {
      return
...
export function extractPosterDetails(
  ...
  const fallbackAlt =
    typeof options.fallbackAlt === 'string' ? options.fallbackAlt : getPageCleanTitle()
  const selectors =
    Array.isArray(options.selectors) && options.selectors.length
      ? options.selectors
...
function extractPosterFromImageElement(
  ...
  const href = anchor
    ? resolveAbsoluteUrl(anchor.getAttribute('href') || anchor.href || '', baseUrl)
    : ''
