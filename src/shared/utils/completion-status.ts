...
export function mergeSeasonCompletionMap(
  ...
  const target: Record<string, CompletionStatus> =
...
export function sanitizeSeasonEntry(entry: SeasonEntryInput): SeasonEntry | null {
  ...
  const seasonId =
    typeof entry.seasonId === 'string' && entry.seasonId
      ? entry.seasonId
...
export function normalizeSeasonEntries(
  entries: SeasonEntryInput[] | null | undefined,
): SeasonEntry[] {
...
export function summarizeSeasonCompletion(
  statuses: CompletionStatusInput[] = [],
): CompletionStatus | null {
